...
#define BLUE_FILTER	0x001F
#define YELLOW_FILTER	0xFFE0
#define CYAN_FILTER	0x07FF
#define MAGENTA_FILTER	0xF81F
#define WHITE_FILTER	0xFFFF
...
int main ( void ) {
	...
	DCMI_Control_IO_Init();
        ...
        LCD_DisplayStringLine ( LINE(2), (uint8_t *)" Р�РґС‘С‚ РёРЅРёС†РёР°Р»РёР·Р°С†РёСЏ " );
        LCD_DisplayStringLine ( LINE(3), (uint8_t *)"      РєР°РјРµСЂС‹...     " );
    ...
    NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
         ...
         LCD_DisplayStringLine ( LINE(2), (uint8_t *)"Р�РЅРёС†РёР°Р»РёР·Р°С†РёСЏ РєР°РјРµСЂС‹" );
         LCD_DisplayStringLine ( LINE(3), (uint8_t *)"     РЅРµ РїСЂРѕС€Р»Р°!!!   " );
         LCD_DisplayStringLine ( LINE(5), (uint8_t *)"РџСЂРѕРІРµСЂСЊС‚Рµ СЃРѕРµРґРёРЅРµРЅРёРµ" );
         LCD_DisplayStringLine ( LINE(6), (uint8_t *)" Рё РїРѕРІС‚РѕСЂРёС‚Рµ РїРѕРїС‹С‚РєСѓ" );
 		/* Go to infinite loop */
 		while (1);
 	}
    ...
    if ( (pstate->X > MIN_X) && (pstate->Y > MIN_Y) ) {
		uint16_t coor_x = (uint16_t)((float)LCD_PIXEL_WIDTH
* (MAX_X - pstate->X) / (MAX_X - MIN_X));
		uint16_t coor_y = (uint16_t)((float)LCD_PIXEL_HEIGHT
...
&& (coor_y < redButton_coor[3])) {
		currentFilter = _Y;
		currentFilter = currentFilter * 0xFF + C_B;
		currentFilter = currentFilter * 0xFF + C_R;
...
(coor_x > 0) && (coor_x < LCD_PIXEL_WIDTH)
 && (coor_y > 0) && (coor_y < 52)) {
		...
		// РџРµСЂРµС…РѕРґ РІ Р±РµСЃРєРѕРЅРµС‡РЅС‹Р№ С†РёРєР» Рё РѕР¶РёРґР°РЅРёРµ
		// РІРЅСѓС‚СЂРµРЅРЅРµРіРѕ СЃР±СЂРѕСЃР° РјРёРєСЂРѕРєРѕРЅС‚СЂРѕР»Р»РµСЂР°
						while (1);
					}
				}
			}
...
void DMA2_Stream1_Handler ( void )
{
	...
	uint16_t * pScreen = (uint16_t *) gImage_BaseImage + screen_index;
			screen_index += 2;
			if (screen_index > screen_WH) {
				screen_index -= screen_WH;
	LCD_WriteReg ( SSD2119_X_RAM_ADDR_REG, 0 );
	LCD_WriteReg ( SSD2119_Y_RAM_ADDR_REG, 0 );
				LCD_WriteRAM_Prepare();
			}
	uint16_t x = screen_index % LCD_PIXEL_WIDTH;
	uint16_t y = screen_index / LCD_PIXEL_WIDTH;
	uint16_t data = (pScreen[0] == data_0) ?
	 (pPixel[0] & currentFilter) : pScreen[0];
	if (currentButton)
	{
		if ((x >= currentButton[0]) && (x <= currentButton[2]) && (y >= currentButton[1]) && (y <= currentButton[3]))
			{
				data &= currentFilter;
			}
	}
	LCD_WriteRAM ( data );
	data = (pScreen[1] == data_0) ? (pPixel[1] & currentFilter) : pScreen[1];
	if (currentButton)
	{
	if ((x >= currentButton[0]) && (x <= currentButton[2]) && (y >= currentButton[1]) && (y <= currentButton[3]))
	{
